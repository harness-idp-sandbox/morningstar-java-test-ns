...
# run app
java -jar target/java-test-ns-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-test-ns:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-test-ns -n dev

# get external endpoint
kubectl get svc java-test-ns-svc -n dev
```
...
```bash
kubectl port-forward svc/java-test-ns-svc 8080:80 -n dev
...
kubectl rollout restart deploy/java-test-ns -n dev
kubectl rollout status deploy/java-test-ns -n dev
```
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javatestns/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© neel.shah@morningstar.com. For demo/PoV purposes.
